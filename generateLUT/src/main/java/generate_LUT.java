...
public class generate_LUT implements PlugIn {
	...
	public int s1=2501,s2=129,s2p=101,thickness,above,below,increment,temp;
	...
	public void run(String arg){
        ...
        double[] guess = {((double)(thickness))/1000,1,0.25};
        final RealVector obs = new ArrayRealVector(xdata), start = new ArrayRealVector(guess);
        final MultivariateJacobianFunction fn = new irisFun(cs,temp);
        ConvergenceChecker<LeastSquaresProblem.Evaluation> cc = new EvaluationRmsChecker(.00001,.001);
        final int itereval=1000000000;
        final LeastSquaresProblem	lsqprob = LeastSquaresFactory.create(fn,obs,start,cc,itereval,itereval);

        final LevenbergMarquardtOptimizer lm=new LevenbergMarquardtOptimizer();
        
        final RealVector value = lm.optimize(lsqprob).getPoint();
	...
	public double[] irisfxn(double start,double m,double b,double temp){
		...
		for(int j=0;j<4;j++) {
			...
			for(double i=.4;i<.651;i+=0.001){
				...
				sirefract2=SiRI(i,temp);
				sio2refract=SiO2RI(i,temp);
